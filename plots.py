...
ax[0].set_xlabel("Limitting factor", fontsize = fs)
ax[1].set_xlabel("Limitting factor", fontsize = fs)
...
plt.xticks([0,ri,1], [r"$f_i(c_jN_j^*,0)$",r"$f_i(0,N_j^*)$",r"$f_i(0,0)$"],
           fontsize = fs)
plt.yticks([0,ri,1], [0,r"$ND$",1],fontsize = fs)
...
plt.axis([0,1,-1,5])
plt.ylabel(r"$-FD$", fontsize = fs)
plt.xlabel(r"$ND$", fontsize = fs)
