...
safety = 2*n_spe_max # assumed maximal resident density factor
...
L_u_r = np.amax(util/(H_sat*regen), axis = 1)
B_u_r = (np.sum(util/H_sat, axis = 1) - 
         np.sum((util/H_sat)**2/regen, axis = 1)/(safety * L_u_r))

H_spec_diag = safety*np.ones((n_spe_max, n_com))
...
u_spec_diag = B_u_r*(1+H_spec_diag)*safety*H_spec_diag/(1-safety+H_spec_diag)
...
r_spec = u_spec_diag/L_u_r/H_spec_diag
...
def res_dens(N, u, r, H, m):
    ...
    # resources density
    try:
        R_star = solver_jurg(holling_type2, 1.1*K*np.ones(n_res),
    ...
    return np.sum(u*R_star/(H+R_star), axis = 1) - m
...
def holling_type2(R, N, r, u, H):
    return r*(1-R/K) - np.sum(N.reshape(-1,1)*u/(R+H), axis = 0)
